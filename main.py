...
def draw_window(lander, thruster_level):
    ...
    lander_surface = pygame.transform.rotate(pygame.transform.scale(
        lander_image, (LANDER_DISPLAY_WIDTH, LANDER_DISPLAY_HEIGHT)), 0)
    WINDOW.blit(lander_surface, (lander.x, lander.y))

    pygame.display.update()


def handle_lander_movement(keys_pressed, lander):
    thruster_level = 0
    ...
    if keys_pressed[pygame.K_s] and lander.y + VELOCITY + lander.height < HEIGHT:  # DOWN
        lander.y += VELOCITY
        thruster_level = 2
    return thruster_level

# Main lunar lander game function
def lunar_lander():
    lander = pygame.Rect(WIDTH//2 - LANDER_WIDTH//2, 20, LANDER_WIDTH, LANDER_HEIGHT)
    ...
    while run:
        ...
        thruster_level = handle_lander_movement(keys_pressed, lander)

        # Update the display to show the current state of the game
        draw_window(lander, thruster_level)
